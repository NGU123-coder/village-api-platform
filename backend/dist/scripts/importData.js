...
// Configuration
const CSV_FILE_PATH = path_1.default.join(__dirname, '../../data-pipeline/data/sample.csv');
const BATCH_SIZE = 2000;
// In-memory cache to avoid redundant DB lookups for parents
const cache = {
    countryId: null,
    states: new Map(), // name -> id
    districts: new Map(), // stateId_name -> id
    subDistricts: new Map(), // districtId_name -> id
};
async function main() {
    ...
    for await (const row of stream) {
        totalRows++;
        try {
            const { state_name, state_code, district_name, sub_district_name, village_name, pin_code, population } = row;
...
async function processVillageBatch(batch) {
    ...
    await prisma.$transaction(batch.map(v => prisma.village.upsert({
